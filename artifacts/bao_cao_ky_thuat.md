...
![Splash Screen](C:\Users\admin\.gemini\antigravity\brain\adc1a94c-25ad-4f80-a1de-49b27887d5bf\artifacts\splash_screen.png)
...
![Onboarding Screen](C:\Users\admin\.gemini\antigravity\brain\adc1a94c-25ad-4f80-a1de-49b27887d5bf\artifacts\onboarding_screen.png)
...
![Home Screen](C:\Users\admin\.gemini\antigravity\brain\adc1a94c-25ad-4f80-a1de-49b27887d5bf\artifacts\home_screen.png)
...
![Game Screen](C:\Users\admin\.gemini\antigravity\brain\adc1a94c-25ad-4f80-a1de-49b27887d5bf\artifacts\game_screen.png)

#### Màn 5: Profile Screen
- **Mô tả:** Thông tin nhóm L02-CockRoaches, tech stack, project goal. Avatar gradient ring.

![Profile Screen](C:\Users\admin\.gemini\antigravity\brain\adc1a94c-25ad-4f80-a1de-49b27887d5bf\artifacts\profile_screen.png)

### 5.3. Demo Animation

![Splash → Onboarding Animation](C:\Users\admin\.gemini\antigravity\brain\adc1a94c-25ad-4f80-a1de-49b27887d5bf\artifacts\splash_animation.webp)
...
|---|---|---|---|---|---|
| `POST` | `/auth/signup` | ❌ | Đăng ký tài khoản | `{email, password, name}` | `{message, userId}` |
| `POST` | `/auth/login` | ❌ | Đăng nhập | `{email, password}` | `{access_token, user}` |
| `POST` | `/auth/google` | ❌ | Google OAuth | `{idToken}` | `{access_token, user}` |
...
| `GET` | `/games/:id` | ❌ | Chi tiết một trò chơi |

#### Scores Module (`/v1/scores`)

| Method | Endpoint | Auth | Mô tả | Request Body |
|---|---|---|---|---|
| `POST` | `/scores` | ✅ Bearer | Lưu điểm mới | `{value, accuracy?, timeSpent?, gameId}` |
| `GET` | `/scores/leaderboard?gameId=` | ❌ | Top 10 leaderboard | - |
| `GET` | `/scores/me` | ✅ Bearer | Điểm cao cá nhân | - |

#### Matchmaking Module (`/v1/matchmaking`)

| Method | Endpoint | Auth | Mô tả |
|---|---|---|---|
| `POST` | `/matchmaking/join` | ✅ | Tham gia hàng chờ |
| `GET` | `/matchmaking/status?gameId=&allowBot=` | ✅ | Kiểm tra trạng thái |
| `POST` | `/matchmaking/leave` | ✅ | Rời hàng chờ |
| `POST` | `/matchmaking/room/create` | ✅ | Tạo phòng |
| `GET` | `/matchmaking/rooms` | ✅ | Danh sách phòng công khai |
| `POST` | `/matchmaking/room/join` | ✅ | Vào phòng bằng mã |
| `GET` | `/matchmaking/room/status?roomId=` | ✅ | Trạng thái phòng |
| `POST` | `/matchmaking/room/leave` | ✅ | Rời/Hủy phòng |
...
|---|---|---|
| **Statements** | **38.04%** | 1,958 / 5,146 |
| **Branches** | **72.80%** | 83 / 114 |
| **Functions** | **34.52%** | 29 / 84 |
| **Lines** | **38.04%** | 1,958 / 5,146 |
...
| `app/(tabs)/` (home, profile) | **30.63%** ❌ | **49.12%** ⚠️ | **21.53%** ❌ | **30.63%** ❌ | Cần cải thiện |
| `app/services/` (api.ts) | **20.86%** ❌ | **100%** ✅ | **0%** ❌ | **20.86%** ❌ | Cần cải thiện |
...
| `home-screen.test.tsx` | `app/(tabs)/home.tsx` | 3 | Render, play button nav, mount |
...
**Tổng backend: 3 test cases**

### 9.4. Phân Tích Khoảng Trống (Coverage Gaps)

| Module Chưa Test | Lý Do | Mức Ưu Tiên |
|---|---|---|
| **`services/api.ts`** | 13 hàm API chưa có unit test (chỉ branch coverage do code path analysis) | 🔴 Cao |
| **`home.tsx` modals** | 7 modal overlays + matchmaking logic rất phức tạp | 🟡 Trung bình |
| **`auth.service.ts`** (BE) | Chưa có test cho signup, login, googleLogin | 🔴 Cao |
| **`auth.controller.ts`** (BE) | Chưa có integration/e2e test | 🟡 Trung bình |
| **`scores.service.ts`** (BE) | Chưa test create, getLeaderboard, getMyHighScores | 🔴 Cao |
| **`matchmaking.service.ts`** (BE) | Chưa test queue, room, bot matching logic | 🔴 Cao |
| **`explore.tsx`** (Game) | Excluded — cần Detox/Maestro cho multi-touch test | 🟡 Trung bình |

### 9.5. Đề Xuất Cải Thiện

> [!IMPORTANT]
> Coverage hiện tại (**38.04% statements**) chưa đạt mục tiêu **70%**. Cần tập trung vào các module cốt lõi sau:
